...
AGE_STYLE = {
    "young": (
        ...
        "Think: challenges, reactions, drama, relatable struggles."
    ),
    "general": (
        "Target audience: general adults (25–40). "
        "Use clear, curiosity-driven language. "
        "Think: self-improvement, productivity, money, lifestyle."
    ),
    "older": (
        "Target audience: older adults (40+). "
        "Use trustworthy, informative, straightforward language. "
        "Think: news, finance, health, practical how-to, no slang."
    ),
}
...
# ── Helpers ──────────────────────────────────────────────────────────────────
def brainstorm(idea, demographic="general"):
    style = AGE_STYLE.get(demographic, AGE_STYLE["general"])
    prompt = (
...
def rank_with_model(candidates, demographic="general"):
    model   = models.get(demographic, models["general"])
...
def trained_llm_rank(idea, demographic="general"):
    style  = AGE_STYLE.get(demographic, AGE_STYLE["general"])
    prompt = (
...
def generate():
    ...
    demographic = body.get("demographic", "general")

    if not idea:
        return jsonify({"error": "No idea provided"}), 400

    try:
        candidates = brainstorm(idea, demographic)
        ranked     = rank_with_model(candidates, demographic)
        top_title  = ranked[0][0]

        rationale = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role": "user", "content":
                f"Explain in one punchy sentence why '{top_title}' would perform well "
                f"with a {demographic} audience on YouTube."
            }]
...
def compare():
    ...
    try:
        candidates  = brainstorm(idea, demographic)
        ctr_ranked  = trained_llm_rank(idea, demographic)

        return jsonify({
            "llm_raw":     [{"title": t} for t in candidates],
            "trained_llm": [{"title": t} for t in ctr_ranked]
        })
    except Exception as e:
        return jsonify({"error": str(e)}), 500
